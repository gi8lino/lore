...
type Page = pluginapi.Page

// PageQuery is a bounded search query.
type PageQuery = pluginapi.PageQuery

// PageContent is authorized Markdown source.
type PageContent = pluginapi.PageContent

// NavigationNode is an authorized navigation entry.
type NavigationNode = pluginapi.NavigationNode

// StoredValue distinguishes absent keys from empty values.
type StoredValue = pluginapi.StoredValue

// AttachmentRead requests a bounded attachment range.
type AttachmentRead = pluginapi.AttachmentRead

// Attachment contains authorized attachment bytes and metadata.
type Attachment = pluginapi.Attachment

// Transport enables native tests to supply fake host capabilities. Production
// clients use Lore's WASM transport; permissions remain enforced by the host.
...
// NewClient creates a client. A nil transport uses the Lore host.
func NewClient(transport Transport) Client {
	if transport == nil {
		transport = pluginapi.Call
	}
...
// Log sends a bounded log entry to Lore.
...
func (c Client) Log(message string) error {
	return c.call("log", pluginapi.LogMessage{Message: message}, nil)
}

// Icon renders an icon through the public host capability.
func (c Client) Icon(name string, size int) (string, error) {
	var result string
	err := c.call("icons.render", pluginapi.IconRequest{Name: name, Size: size}, &result)
	return result, err
}
...
func (p PageClient) Get(slug string) (Page, error) {
	var result Page
	err := p.client.call("pages.get", pluginapi.PageRef{Slug: slug}, &result)
	return result, err
}
...
func (p PageClient) Content(slug string) (PageContent, error) {
	...
	err := p.client.call("pages.content", pluginapi.PageRef{Slug: slug}, &result)
	return result, err
}
...
func (v Values) Get(key string) (StoredValue, error) {
	...
	err := v.client.call(v.method+".read", pluginapi.StorageValue{Key: key}, &result)
	return result, err
}

// Set writes opaque bytes. Lore enforces permissions and namespace quotas.
func (v Values) Set(key string, value []byte) error {
	return v.client.call(v.method+".write", pluginapi.StorageValue{Key: key, Value: value}, nil)
}
