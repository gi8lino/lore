...
Variables used only in link destinations, image attributes, or complex Markdown are still listed, even when there is no text that can be highlighted safely. Lore keeps the normal rendered document whenever annotations would change its content or structure.
