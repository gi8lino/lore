...
| Knowledge                    | Saved-search validation → 422, duplicate name → 409, missing owned search → 404. Snippet validation → 422 and missing/duplicate snippets → 404/409 through the admin translator. Graph, snippet lists, saved-search lists, and notifications have infrastructure fallbacks. Marking a missing notification read is idempotent. Missing includes/snippets are handled by Markdown expansion. |
