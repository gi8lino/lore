...
`POST /export/preview/{slug...}` returns `{"document":"..."}` containing the clean print document. `POST /export/pdf/{slug...}` returns the generated PDF. Both use private, non-cacheable responses. Malformed requests return 400; invalid or unused variable overrides return 422 through Lore's field-problem format. Existing `GET /export/pdf/{slug...}` links continue to use saved values; URL parameters do not set overrides. These are browser export routes, not bearer-token API routes.
