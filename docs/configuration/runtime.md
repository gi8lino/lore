...
Lore uses command-line flags and matching `LORE__` environment variables. The database URL is required for the normal server mode; it is not required for `lore site build`.
