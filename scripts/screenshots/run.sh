...
work_dir=$(mktemp -d "${TMPDIR:-/tmp}/lore-screenshots.XXXXXX")
archive="$work_dir/documentation.zip"
binary="$work_dir/lore"
server_log="$work_dir/lore.log"
server_pid=""
...
"$binary" serve \
  ...
  --public-url="$base_url" \
  --database-url="postgres://lore:lore@127.0.0.1:$database_port/lore?sslmode=disable" \
