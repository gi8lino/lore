// Package portable defines the Kumbuka-compatible portable archive format used for migration.
package portable

const (
	// Format identifies a Kumbuka portable archive manifest.
	Format = "kumbuka"
	// Version is the portable archive schema version produced by Lore.
	Version = 1
	// ManifestPath is the required root-level archive manifest.
	ManifestPath = "manifest.json"
)

// Manifest inventories every portable page and bundled resource in an archive.
type Manifest struct {
	// Format identifies the archive contract and must equal Format.
	...
	// Media contains uploaded images included in the archive.
	Media []ResourceEntry `json:"media,omitempty"`
	// Attachments contains uploaded files included in the archive.
...
// PageMetadata contains page settings that remain meaningful when imported into Kumbuka.
type PageMetadata struct {
	// Slug is the canonical page path and must match the manifest entry.
	Slug string `json:"slug"`
...
// NewManifest creates an empty manifest for the Kumbuka-compatible archive format.
