...
func TestReadExportParameters(t *testing.T) {
	...
	t.Run("preserves explicit empty values", func(t *testing.T) {
		t.Parallel()
		request := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(`{"parameters":{"io.lore.variables":{"values":{"environment":""}}}}`))
		values, err := readExportParameters(httptest.NewRecorder(), request)
		require.NoError(t, err)
		assert.Equal(t, "", values["io.lore.variables"]["values"]["environment"])
	})

	t.Run("ignores bodies on get", func(t *testing.T) {
		t.Parallel()
		request := httptest.NewRequest(http.MethodGet, "/", strings.NewReader(`{"parameters":{"io.lore.variables":{"values":{"environment":"staging"}}}}`))
		values, err := readExportParameters(httptest.NewRecorder(), request)
		require.NoError(t, err)
		assert.Nil(t, values)
	})
...
func TestValidateExportParameters(t *testing.T) {
	...
	assert.NoError(t, validateExportParameters(map[string]map[string]map[string]string{
		"io.lore.variables": {"values": {"environment": " staging \n"}},
	}))
