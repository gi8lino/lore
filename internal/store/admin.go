...
func (s *Store) SaveRenderingSettings(ctx context.Context, settings domain.RenderingSettings) error {
	_, err := s.pool.Exec(ctx, `
UPDATE application_settings
SET render_wiki_links=$1,
	    render_tabs=$2,
	    render_details=$3,
	    render_strikethrough=$4,
	    render_task_lists=$5,
	    render_autolinks=$6,
	    render_syntax_highlighting=$7,
	    render_footnotes=$8,
	    render_definition_lists=$9,
	    render_typographer=$10,
    updated_at=now()
WHERE singleton=true`,
		settings.WikiLinks,
		settings.Tabs,
