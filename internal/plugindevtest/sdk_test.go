...
// TestExternalSDKProject exercises the real CLI outside Lore's module, then loads
...
func TestExternalSDKProject(t *testing.T) {
	...
	temp := t.TempDir()
	cli := filepath.Join(temp, "lore-plugin")
	command := exec.Command("go", "build", "-o", cli, "./cmd/lore-plugin")
	command.Dir = root
	...
	// Include hostile HTML to prove SDK output still crosses Lore's sanitizer.
	...
	run(project, "test")
	run(project, "build")
	filename := filepath.Join(project, "dist", "my-plugin.loreplugin")
