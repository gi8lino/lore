...
func TestUpgradeRetainsAcquiredVersionUntilRenderFinishes(t *testing.T) {
	...
	require.NoError(t, err)
	snapshot, release := registry.Acquire()
	defer release()
	next := changedManifest(t, data, func(s string) string { return strings.ReplaceAll(s, "1.0.0", "2.0.0") })
	_, err = manager.Upgrade(ctx, metadata.Manifest.ID, next)
	require.NoError(t, err)
	source := "Ordinary text"
	got, err := snapshot.Entries[0].Contributions.Preprocessors[0].Preprocess(plugin.Context{}, source)
	require.NoError(t, err)
...
func TestUpgradeDuringRenderingKeepsWholeSnapshotAlive(t *testing.T) {
	...
	<-blocker.start
	next := changedManifest(t, data, func(s string) string { return strings.ReplaceAll(s, "1.0.0", "2.0.0") })
	_, err = manager.Upgrade(ctx, "io.lore.callouts", next)
...
func TestLifecycleDependenciesAndFailedBootstrapAreAtomic(t *testing.T) {
	...
	child := changedManifest(t, data, func(s string) string {
		return strings.ReplaceAll(s, "io.lore.callouts", "io.child") + "\nrequires: [io.base]\n"
	})
