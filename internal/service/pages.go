...
type pageRepository interface {
	AddPageComment(context.Context, string, int64, string, string) (PageComment, error)
	...
	MovePage(context.Context, string, string, MovePageOptions, User) error
	...
	SavePage(context.Context, string, string, string, string, string, string, string, []string, []string, []int64, PageMetadata, map[string]string, User) (Page, error)
}
...
func (s *Pages) Delete(ctx context.Context, slug string, actor User) error {
...
func (s *Pages) Review(ctx context.Context, slug string, actor User) error {
	slug = strings.TrimSpace(slug)
...
func (s *Pages) RestoreRevision(ctx context.Context, slug string, number int, actor User) (Page, error) {
	if number <= 0 {
...
func (s *Pages) AddComment(ctx context.Context, slug, anchor, body string, actor User) error {
...
func (s *Pages) Import(ctx context.Context, candidates []ImportedPage, format string, actor User) (int, error) {
...
func (s *Pages) importPage(ctx context.Context, candidate ImportedPage, actor User) error {
...
func (s *Pages) bulkMove(ctx context.Context, slugs []string, target string, actor User) error {
	...
	for _, slug := range orderedSlugs {
		...
		if err := s.repository.MovePage(
			...
			slug,
			destination,
			MovePageOptions{UpdateIncomingLinks: true, KeepAliases: true},
			actor,
		); err != nil {
			return err
		}
