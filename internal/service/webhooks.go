...
func validateWebhookBodyTemplate(value string) error {
	...
	body, err := tmpl.Render(webhookNotification{event: event, publicURL: "https://lore.example"}.Data("Example", nil, event.Event))
	if err != nil {
		...
		if strings.TrimSpace(value) == "" {
			...
			if input.Sensitive {
				if s.secrets == nil || !s.secrets.Configured() {
					return nil, domain.NewValidationError("headers", "Configure LORE__ENCRYPTION_KEY before saving sensitive webhook headers.")
				}
...
func (s *Webhooks) RevealWebhookHeader(ctx context.Context, webhookID, headerID int64) (string, error) {
	...
	for _, header := range item.Headers {
		...
		if s.secrets == nil || !s.secrets.Configured() {
			return "", domain.NewValidationError("headers", "Configure LORE__ENCRYPTION_KEY before revealing sensitive webhook headers.")
		}
...
func (s *Webhooks) deliver(ctx context.Context, item domain.Webhook, event OutgoingEvent) error {
	...
	titleTemplate, err := templates.ParseStringTemplate("lore-webhook-title", `{{ .Input.Event }}`, templates.WithDefaultFuncs())
	if err != nil {
		s.recordWebhookDelivery(ctx, item.ID, event.Event, 0, 0, err.Error())
		return err
	}
...
// webhookRequestHeaders decrypts sensitive headers and adds Lore request metadata.
...
func parseWebhookBodyTemplate(value string) (*templates.Template, error) {
	return templates.ParseTemplate("lore-webhook-body", value, templates.WithDefaultFuncs())
}
...
// webhookNotification adapts a Lore event to Notifykit's Notification contract.
