...
func TestFrameEscapesAssetNamesAndKeepsExecutionIsolated(t *testing.T) {
	frame, policy, err := Frame("/plugins", "/plugins/runtime.js", []string{"https://lore.test"}, plugin.BrowserContribution{PluginID: "io.example.test", ModuleID: "diagram", Digest: "abc", Name: "<script>alert(1)</script>", JavaScript: "entry#special?.js", CSS: "style%.css"})
