...
test("plugin administration forms drive the real runtime lifecycle", async () => {
  const directory = await mkdtemp(join(tmpdir(), "lore-plugin-admin-"));
  let server, browser;
  try {
    ...
    await installForm.locator("[data-plugin-upload-dropzone]").evaluate((dropzone) => {
      ...
      transfer.items.add(
        new File(["fixture"], "dropped.loreplugin", { type: "application/zip" }),
      );
    ...
    assert.equal(
      ...
      "Choose a .loreplugin package.",
    );
    assert.equal(await upgradeButton.isDisabled(), true);

    await upgradeForm.locator("[data-plugin-upload-input]").setInputFiles({
      name: "broken.loreplugin",
      mimeType: "application/zip",
      buffer: Buffer.from("invalid"),
    });
    ...
    const denied = await page.request.post(
      url + "/admin/plugins/io.lore.tables/disable",
