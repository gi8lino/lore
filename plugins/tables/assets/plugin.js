...
(() => {
    ...
    function compareTableRows(left, right, column, direction) {
        const result = tableCollator.compare(tableCellText(left, column), tableCellText(right, column));
        return direction === "ascending" ? result : -result;
    }
    function tableControlIcon(pathData) {
    ...
    function setupTableSorting(table, headers, body) {
        for (const [column, header] of headers.entries()) {
            const button = header.querySelector(":scope .table-sort-button");
            ...
            button.addEventListener("click", () => {
                ...
                for (const candidate of headers) {
                    ...
                    const candidateIndicator = candidate.querySelector(":scope .table-sort-indicator");
                    if (candidateIndicator)
                        candidateIndicator.textContent = "↕";
                }
                header.setAttribute("aria-sort", direction);
                indicator.textContent = direction === "ascending" ? "↑" : "↓";
                const rows = tableRows(table);
                rows.sort((left, right) => compareTableRows(left, right, column, direction));
                for (const row of rows)
                    body.append(row);
            });
        }
    }
    function setupTableFiltering(table, shell, headers, filterButtons) {
        const rows = tableRows(table);
        ...
        searchIcon.append(tableControlIcon("M21 21l-4.35-4.35M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0Z"));
        ...
        function openMenu(column, button) {
            ...
            activeColumn = column;
            activeButton = button;
            const columnLabel = headers[column]?.dataset.tableColumnLabel || `Column ${column + 1}`;
        ...
        for (const [column, button] of filterButtons.entries()) {
            button.addEventListener("click", () => {
                if (!menu.hidden && activeButton === button)
                    closeMenu();
                else
        ...
        input.addEventListener("input", () => {
            ...
            if (query === "")
                ...
                filters.set(activeColumn, query);
            clear.hidden = query === "";
            applyFilters();
        });
        ...
        shell.addEventListener("pointerdown", (event) => {
            ...
            if (menu.hidden || !(target instanceof Element) || menu.contains(target))
                return;
            const filterButton = target.closest(".table-filter-button");
            if (filterButton && filterButtons.includes(filterButton))
                return;
            closeMenu();
        });
        window.addEventListener("resize", () => {
            if (!menu.hidden && activeButton)
                positionMenu(activeButton);
        });
        shell
            .querySelector(":scope > .lore-table-scroll")
    ...
    function setupTableHeaders(table, shell, sortable, filterable) {
        ...
        for (const [column, header] of headers.entries()) {
            ...
            if (filterable) {
                ...
                filterButton.append(tableControlIcon("M22 3H2l8 9.46V19l4 2v-8.54L22 3Z"));
        ...
        if (sortable)
            setupTableSorting(table, headers, body);
        if (filterable)
            setupTableFiltering(table, shell, headers, filterButtons);
    }
    function setupInteractiveTable(table) {
        if (table.dataset.loreInteractiveReady === "true")
            return;
